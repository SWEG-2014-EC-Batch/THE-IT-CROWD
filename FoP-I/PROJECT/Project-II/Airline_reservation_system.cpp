...
struct customer {
	...
	char yes_no;
	void main_choice1(customer[]), main_choice2(customer[]), main_choice3(customer[]), detailsInput(customer& ), detailsOutput(customer& person, int& seatNum, string section), display2(customer[], int),write(customer), read(customer person[]);
...
choice :
				...
				if(sub_choice==1) {
					...
					if(first_class>=30) {
						...
						if(toupper(yes_no) == 'Y') {
							...
							person[first_class].flightType='F';
							write(person[first_class]);
				...
				// code to make reservation class for economy class
					...
					if(economic_class>=100) {
						...
						person[economic_class].flightType='E';
						write(person[economic_class]);
						//display boarding pass according to the user information
...
void main_choice2(customer person[]){
    ...
    int i =0;
        while(file.eof() == false){
            file>>person[i].passport>>person[i].name>> person[i].age>>person[i].sex;
            i++;
        }
    }
    file.close();
...
void write(customer person){
    fstream file;
    (person.flightType=='F')?(file.open("files/firstClassList.txt",ios::in|ios::out)):(file.open("files/economicClassList.txt",ios::in|ios::out));
    if(!file.is_open()){
        cout<<"\acould not be opened"<<endl;
    }
    else{
        file<<person.passport<<" "<<person.name<<" "<<person.age<<" "<<person.sex<<endl;
    }
    file.close();
}

void read(customer person[]){
    fstream file;
    file.open("files/firstClassList.txt",ios::in|ios::out);
    if(!file.is_open()){
        cout<<"\acould not be opened"<<endl;
